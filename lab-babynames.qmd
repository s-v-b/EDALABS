...
df_lorenz_fr |> 
  ...
  group_by(sex, name) |>
  arrange(year) |>
  mutate(sp=slide_vec(p, mean, .before = 2, .after = 2, .complete = T)) |>
  ungroup() |> 
  filter(sex=='M', name %in% c('STÉPHANE', 'KEVIN', 'ENZO', 'THÉO', 'GABRIEL')) |> 
  ggplot() +
...
```{r}
df_ratio_pop |> 
  filter(name %in% c('STÉPHANE', 'ENZO', 'KEVIN', 'THÉO'))
```
...
```{r}
(df_ratio_pop|> 
  filter(name %in% pull(filter(df_nouvelle_france, sex=='M'), name), sex=='M') |> 
  ggplot() +
  aes(x=ryear, y=sp,  shape=name, linetype=name, label=ymax) +
  geom_line() +
  scale_y_log10() +

...
Plot the proportion of female births given name 'MARIE' or 'MARIE-...' as a fucntion of year. Proceed in such a way that the reader can see the share of compounded names. We are expecting an _area plot_
...
```
1. `%T>%` Tee-pipe offered by `magrittr`: The tee pipe works like `%>%`, except the return value is lhs itself, and not the result of rhs function/expression.
2. `<<-` is the *scoping assignment*. Here it makes `maxshare` avalaible in the calling environment of calls to `annotate()`
3. This should not be necessary. Inconsistency in quarto ?


See Graphique 3, page 48, de _L'archipel français_  de J. Fourquet. Le Seuil. Essais. Vol. 898. 


See [Relative coordinates or NPC in ggplot2](https://www.christophenicault.com/post/npc_ggplot2/)

[tee pipe in magrittr documentation](https://magrittr.tidyverse.org/reference/tee.html#:~:text=Pipe%20a%20value%20forward%20into,effect%2C%20say%20plotting%20or%20printing.)

[scoping assignment in ]()
...
#| eval: true
p_jeanne <- p_marie %+% (  
  share_name(df_fr, .name_stem = "JEANNE") %T>% 
  {maxshare <<- max(pluck(., "share") , na.rm=T)}
  ) + 
  ggtitle("JEANNE")
```
...
#| eval: true
p_france <- p_marie %+% (  
  share_name(df_fr, .name_stem = "FRANCE") %T>% 
  {maxshare <<- max(pluck(., "share") , na.rm=T)}) + 
  ggtitle("FRANCE")
```

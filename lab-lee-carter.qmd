...
```{r compute_list_lis, echo=FALSE}
pcas_by_country_gender %>%
  purrr::map( ~ add_column(.[['value']][['li']], Year=year_p:year_e)) -> list_df_li
```

```{r gglicircle, echo=FALSE, eval=TRUE}
...
# multiplot(plotlist = list_screeplots_ns, cols = 2, byrow = TRUE)
```
...
qplot(x=1:100, y=EnWales_2[66,]) +
  geom_line() 
>>>>>>> ca46ad21d5b5d71bd8dc4f58e924101a45228507
```

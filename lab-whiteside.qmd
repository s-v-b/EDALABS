...
These diagnostic plots can be built from the information gathered in the `lm` object returned by `lm(...)`. It is convenient to extract the required pieces of information using method `augment.lm`. of generic function `augment()` from package `broom`. 
...
diag_1 <- whiteside_aug %>% 
  ...
  xlab("Fitted values") +
  ylab("Residuals)") +
  # ggtitle(glue("{cur_dataset} data, regression Gas ~ Temp"),
  #         subtitle = "") +
...
Understanding `.sigma`: check the documentation 

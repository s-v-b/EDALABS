...
Plot central death rates against ages using a logarithmic scale on the
$y$ axis. Countries are identified by aesthetics (shape, color, linetypes). Abiding to the DRY principle, define
a prototype `ggplot`  (alternatively `plotly`) object. 

The prototype will be fed with different datasets and decorated
...
During the last seventy years, death rates decreased at all ages in all seven countries.
...
# Life expectancy, window functions `reduce` and `accumulate`
...
compute a second window function. 
...
The resulting table requires a substantial amount of computation.


::: {.callout-tip}


The official calculation assume that except  at age $0$ and great age, 
...
```{r rlex, warning=FALSE, message=FALSE}
rlex <- function(z) {
  accumulate(z,
...
df_rle |>
  filter(Age < 10, 
         Gender == 'Male', 
         Year == 2016) |>
  mutate(`Residual Life Expectancy`=format(`ex`, digits = 4)) |>
...
df_rle |>
  ...
  ggplot(aes(x=Year, y=ex, group=Age, color=as_factor(Age))) +
  labs(color="Age") +
  ylab("Residual life expectancy") +
  geom_line() +
  facet_grid(rows=vars(Country), cols=vars(Gender)) +
  ggtitle("Evolution of residual life expectancy")
```
...
{ df_pension %>%
  ...
  geom_line(size=.2) +
  facet_grid(cols=vars(Gender)) +
  ggtitle("Residual Life Expectancy takes off")  } %>% ggplotly()
```
...
{ df_pension   %>%
  ...
  ylim(c(60,100)) +                                                                                                                                                                           
  geom_line(size=.2) +
  facet_grid(cols=vars(Gender)) } %>% ggplotly()
```

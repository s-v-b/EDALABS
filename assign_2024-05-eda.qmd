...
![](./IMG/UniversiteParis_monogramme_couleur_RVB.png){style="float:right; size: 50px;"
width="132"}
...
1. Vérifier que si $\|.\|_i$ et $\|.\|_o$ désignent des normes sur $\mathbb{R}^n$ et $\mathbb{R}^p$ alors $f(M) = \sup_{x \in \mathbb{R}^p\setminus \{0\}} \|M x \|_o/\|x\|_i$ définit une norme sur $\mathcal{M}_{n,p}$. On dit que $f$ est la norme ubordonnée à  $\|.\|_i$ et $\|.\|_o$
2. $\|.\|_{\text{op}}$ est-elle une norme subordonné ? si oui à quoi ?
3. Même question pour $\|.\|_{\text{HS}}$
4. Est-il vrai que $\|X\|_{\text{op}} \leq \|X\|_{\text{HS}} \leq max(n,p) \|X\|_{\text{op}}$?

...
format:
  ...
  pdf:
    include-in-header:
      - text: "\\lhead{{\\sf  EDA \\\\ R Introduction}}"
...
we assign dataframe `gapminder` from package `gapminder` to identifier `"gapminder"`  in global environment  {{< fa meh-rolling-eyes >}}. 
...
```
A very nice feature of `R` is that functions from base `R` as well as from packages have *optional* arguments with sensible *default* values. Look for example at documentation of `require()` using expression
`?require`.
...
A square matrix is symmteric iff it is equal to its transpose. 

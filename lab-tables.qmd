...
In table `flights`, `r nrow(filter(flights, is.na(dep_time)))` rows have a missing `dep_time` field. 
...
flights %>% 
  ...
  group_by(carrier) %>% 
  summarise(n_dest=n(), .groups=NULL) %>% 
  filter(n_dest==nrow(distinct(flights, dest))) %>% 
  select(carrier)
```
